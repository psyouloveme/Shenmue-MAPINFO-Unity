    ...
    using (BinaryReader reader = new BinaryReader(s, Encoding.ASCII)) {
      ...
      cols.Colis.ForEach(h => {
        ...
        h.ColiObjs.ForEach(j => {
          ...
          if ((j.ColiType == 0) || (j.ColiType == 0x64)) {
            if (j.ColiType == 0x64) {
              switch (j.ColiSubType) {
                case 0x01: {
                      ColiTypeSixFourZeroOne c = (ColiTypeSixFourZeroOne)j;
                      ...
                      foreach (var v in c.Points)
                      {
                          ...
                          {
                              var g = QuadFactory.MakeMesh(prev, pos, obj, "Quad" + coliObjId, Color.yellow);
                              Debug.DrawLine(prev, pos, Color.yellow, 60);
                          }
                          prev = pos;
                          coliObjId++;
                      }
